    ...
    return (
        <div className="flex-1 min-h-screen flex justify-center items-center">
            ...
            <form
                ...
                className={` space-y-2 w-full ${isMobileView ? 'px-8 text-[#035fc9] bg-white' : 'px-8 text-white'}`}
            >
                {
                    isMobileView && <Link to={'/'} className='top-8 absolute'><FaChevronLeft /></Link>
                }
                <div>
                ...
                <div className=" space-y-2 md:space-y-4">
                    {
                        isMobileView ?
                            <Dropdown isMobile={isMobileView} label={'Account Type*'} selected={accountType} setSelected={setAccountType} options={accountTypes} />
                            :
                            <div className='flex gap-4'>
                                <label className="block md:text-xl bg-white text-[#035fc9] px-4 rounded-md">Select Account Type*</label>
                                <div className="flex gap-4">
                                    {accountTypes.map(type => (
                                        <label key={type.id} className={`flex justify-center items-center px-4 rounded-sm text-white ${accountType.id === type.id ? "bg-[#603504] border-2" : "bg-[#D38A34]"}`}>
                                            <input
                                                type="radio"
                                                name="verification"
                    ...
                    {
                        isMobileView ?
                            <Dropdown isMobile={isMobileView} label={'Verify With*'} selected={selectedVerification} setSelected={setSelectedVerification} options={verificationMethods} />
                            :
                            <div className='flex gap-4'>
                                <label className="block md:text-xl bg-white text-[#035fc9] px-4 rounded-md">Verify with*</label>
                                <div className="flex gap-4">
                                    {verificationMethods.map(method => (
                                        <label key={method.id} className={`flex justify-center items-center px-4 rounded-sm text-white ${selectedVerification.id === method.id ? "bg-[#603504] border-2" : "bg-[#D38A34]"}`}>
                                            <input
                                                type="radio"
                                                name="verification"
                ...
                <div className="col-span-1 md:col-span-2 flex flex-col gap-1">
                    ...
                    <button
                        type="submit"
                        className={`w-full ${isMobileView ? "bg-[#035fc9] hover:bg-[#035fc9be] text-white py-1 text-sm" : "bg-[#D48A35] hover:bg-amber-600 py-2 text-xl"} transition duration-100 rounded-md`}
                    >
                        Create
                    </button>
                </div>
